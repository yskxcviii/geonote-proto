...
function getRailwayColor(name) {
  return COMPANY_COLORS[name] || "#666666";
}
...
function getColor(code) {
  ...
  if (!colorMap[code]) {
    colorMap[code] = COLORS[colorIdx % COLORS.length];
    colorIdx++;
  }
...
function initWorker() {
  try {
    var src =
      "self.onmessage=function(e){try{self.postMessage({i:e.data.i,r:JSON.parse(e.data.t)})}catch(x){self.postMessage({i:e.data.i,e:x.message})}};";
    ...
    parseWorker.onmessage = function (e) {
      ...
      if (e.data.e) cb.reject(new Error(e.data.e));
      else cb.resolve(e.data.r);
    };
  } catch (ex) {
    parseWorker = null;
  }
  ...
  try {
    ...
    if (adminItems.length > 0) {
      ...
      document.getElementById("status").textContent =
        totalFeatures.toLocaleString() + " 個のフィーチャー (キャッシュ)";
    }
    ...
    if (railwayItems.length > 0) {
      ...
      for (var i = 0; i < railwayItems.length; i++) {
        document.getElementById("loaderText").textContent =
          "鉄道路線 (キャッシュ " + (i + 1) + "/" + railwayItems.length + ")";
        ...
        if (geojson.features) {
          ...
          for (var j = 0; j < geojson.features.length; j++) {
            var nm =
              ...
              geojson.features[j].properties.name;
            if (nm && !seen[nm]) {
              seen[nm] = true;
              loadedCompanies.push(nm);
            }
          }
        }
        if (i % 4 === 3) await sleep(0);
      }
      document.getElementById("railwayStatus").textContent =
        railwayItems.length + " ファイル (" + rTotal + " 路線) (キャッシュ)";
      buildRailwayToggles();
    }
    ...
    if (stationItems.length > 0) {
      ...
      var sTotal = 0;
      var seenCo = {};
      for (var x = 0; x < loadedCompanies.length; x++)
        seenCo[loadedCompanies[x]] = true;
      for (var i = 0; i < stationItems.length; i++) {
        document.getElementById("loaderText").textContent =
          "駅 (キャッシュ " + (i + 1) + "/" + stationItems.length + ")";
        ...
        if (geojson.features) {
          ...
          for (var j = 0; j < geojson.features.length; j++) {
            var co =
              geojson.features[j].properties &&
              geojson.features[j].properties.company;
            if (co && !seenCo[co]) {
              seenCo[co] = true;
              loadedCompanies.push(co);
            }
          }
        }
        if (i % 4 === 3) await sleep(0);
...
async function populateCacheList() {
  ...
  try {
    ...
    if (list.length === 0) {
      el.innerHTML =
        '<div style="font-size:12px;color:#aaa;padding:4px 0;">キャッシュなし</div>';
    ...
    for (var cat in categories) {
      ...
      for (var j = 0; j < categories[cat].length; j++) {
        ...
        html +=
          ...
          '<span class="cache-name" title="' +
          item.name +
          '">' +
          item.name +
          "</span>" +
          '<span class="cache-meta">' +
          formatCacheSize(item.size) +
          "</span>" +
          '<button class="cache-del" onclick="deleteCacheItem(decodeURIComponent(\'' +
    ...
    html +=
      '<div style="font-size:10px;color:#aaa;margin-top:6px;text-align:right;">合計: ' +
      formatCacheSize(totalSize) +
      "</div>";
    el.innerHTML = html;
  } catch (e) {
    el.innerHTML =
      '<div style="font-size:12px;color:#d32f2f;">キャッシュ読み込み失敗</div>';
  }
}
...
async function listDriveFolder(folderId) {
  ...
  return (data.files || []).filter(function (f) {
    return f.name.endsWith(".geojson");
  });
}
...
function createPinIcon(color) {
  var svg =
    ...
    '<path d="M10 0C4.5 0 0 4.5 0 10c0 7.5 10 18 10 18s10-10.5 10-18C20 4.5 15.5 0 10 0z" fill="' +
    color +
...
function clearMarkerSelection() {
  ...
  var m = userMarkers.find(function (x) {
    return x.id === selectedMarkerId;
  });
...
function syncMapMarkers() {
  ...
  for (var id in gMapMarkers) {
    if (!idSet[id]) {
      gMapMarkers[id].setMap(null);
      delete gMapMarkers[id];
    }
  }
    ...
    if (gMapMarkers[m.id]) {
      ...
      (function (mid) {
        gm.addListener("click", function () {
          clearSelection();
          selectMarker(mid);
          var cur = userMarkers.find(function (x) {
            return x.id === mid;
          });
          if (cur) {
...
function renderMarkerList() {
  ...
  titleEl.textContent =
    "マーカー" +
    (userMarkers.length > 0 ? " (" + userMarkers.length + "件)" : "");
  if (userMarkers.length === 0) {
    el.innerHTML =
      '<div style="font-size:11px;color:#aaa;padding:6px 0;">マーカーなし</div>';
    return;
  }
  var html = "";
  for (var i = 0; i < userMarkers.length; i++) {
    ...
    var esc = m.title
      ...
      .replace(/"/g, "&quot;");
    html +=
      '<div class="marker-item" onclick="panToMarker(\'' +
      m.id +
      "')\">" +
      ...
      "<button onclick=\"event.stopPropagation();openMarkerModal('" +
      m.id +
      '\')" title="編集">&#9998;</button>' +
      "<button onclick=\"event.stopPropagation();deleteUserMarker('" +
      m.id +
      '\')" title="削除">&times;</button>' +
      "</span></div>";
  }
...
function deleteUserMarker(id) {
  ...
  userMarkers = userMarkers.filter(function (m) {
    return m.id !== id;
  });
  saveUserMarkers();
  syncMapMarkers();
...
function openMarkerModal(id) {
  var editing = id
    ? userMarkers.find(function (m) {
        return m.id === id;
      })
    : null;
  document.getElementById("mEditId").value = editing ? editing.id : "";
  document.getElementById("mAddress").value = editing
  ...
  document.getElementById("markerModalTitle").textContent = editing
    ? "マーカーを編集"
    : "マーカーを追加";
  renderColorPicker();
...
function renderColorPicker() {
  ...
  for (var i = 0; i < MARKER_COLORS.length; i++) {
    ...
    html +=
      '<div class="m-color-opt' +
      sel +
      '" style="background:' +
      c.value +
      ';" title="' +
      c.name +
      '" onclick="selectMarkerColor(\'' +
      c.value +
      "')\"></div>";
  }
...
async function searchMarkerAddress() {
  ...
  var parts = input.split(/[,\s]+/).map(function (s) {
    return parseFloat(s);
  });
  if (
    parts.length >= 2 &&
    ...
    resultEl.textContent =
      "座標: " + parts[0].toFixed(6) + ", " + parts[1].toFixed(6);
    return;
  }
  ...
  try {
    ...
    resultEl.textContent =
      res.addr + " (" + res.lat.toFixed(6) + ", " + res.lng.toFixed(6) + ")";
  } catch (e) {
...
function saveMarkerForm() {
  ...
  if (editId) {
    var idx = userMarkers.findIndex(function (m) {
      return m.id === editId;
    });
    if (idx >= 0) {
...
function openSettings() {
  document.getElementById("sAdminUrl").value =
    localStorage.getItem("map_adminDriveUrl") || "";
  document.getElementById("sRailwayUrl").value =
    localStorage.getItem("map_railwayDriveUrl") || "";
  document.getElementById("sStationUrl").value =
    localStorage.getItem("map_stationDriveUrl") || "";
  populateCacheList();
...
function saveSettings() {
  ...
  if (a) localStorage.setItem("map_adminDriveUrl", a);
  else localStorage.removeItem("map_adminDriveUrl");
  if (r) localStorage.setItem("map_railwayDriveUrl", r);
  else localStorage.removeItem("map_railwayDriveUrl");
  if (s) localStorage.setItem("map_stationDriveUrl", s);
  else localStorage.removeItem("map_stationDriveUrl");
  closeSettings();
  fetchAllFromDrive();
}
...
async function fetchAdminFromDrive() {
  ...
  try {
    ...
    cachePut({
      key: "drive:admin:" + fileId,
      name: "admin_" + fileId.substring(0, 8) + ".geojson",
      category: "admin",
      source: "drive",
      text: text,
      cachedAt: Date.now(),
    ...
    document.getElementById("status").textContent =
      count.toLocaleString() + " 個のフィーチャー (Drive)";
    buildLegend(geojson);
    addMunicipalityLabels(geojson);
  } catch (e) {
    document.getElementById("status").textContent =
...
async function fetchRailwayFromDrive() {
  ...
  try {
    ...
    for (var i = 0; i < files.length; i++) {
      try {
        document.getElementById("loaderText").textContent =
          "鉄道路線 (" + (i + 1) + "/" + files.length + ") " + files[i].name;
        var text = await fetchDriveFile(files[i].id);
        cachePut({
          key: "drive:railway:" + files[i].id,
          name: files[i].name,
          category: "railway",
          source: "drive",
          text: text,
...
async function fetchStationFromDrive() {
  ...
  try {
    ...
    for (var i = 0; i < files.length; i++) {
      try {
        document.getElementById("loaderText").textContent =
          "駅 (" + (i + 1) + "/" + files.length + ") " + files[i].name;
        var text = await fetchDriveFile(files[i].id);
        cachePut({
          key: "drive:station:" + files[i].id,
          name: files[i].name,
          category: "station",
          source: "drive",
          text: text,
...
function clearSelection() {
  if (selectedFeature) {
    if (selectedLayer === "admin") map.data.revertStyle(selectedFeature);
      ...
      railwayLayer.revertStyle(selectedFeature);
    else if (selectedLayer === "station")
      stationLayer.revertStyle(selectedFeature);
...
window.initMap = function () {
  ...
  map.addListener("zoom_changed", function () {
    ...
    if (showLabel !== prevZoomVis.label) {
      ...
      for (var i = 0; i < labelOverlays.length; i++) {
        if (labelOverlays[i].div)
          labelOverlays[i].div.style.display = showLabel ? "" : "none";
      }
    }
  ...
  map.data.addListener("mouseover", function (e) {
    ...
    var parts = [
      e.feature.getProperty("N03_003"),
      e.feature.getProperty("N03_004"),
    ].filter(Boolean);
    if (parts.length === 0)
      parts = [e.feature.getProperty("N03_001")].filter(Boolean);
  ...
  map.data.addListener("click", function (e) {
    ...
    var f = e.feature;
    var parts = [
      f.getProperty("N03_001"),
      f.getProperty("N03_002"),
      f.getProperty("N03_003"),
      f.getProperty("N03_004"),
    ].filter(Boolean);
    document.getElementById("infoName").textContent = parts.join(" ");
    document.getElementById("infoDetail").textContent =
      "行政区域コード: " + (f.getProperty("N03_007") || "-");
  ...
  railwayLayer.setStyle(function (feature) {
    ...
    if (hiddenRoutes[name] || map.getZoom() < ZOOM_RAILWAY)
  ...
  stationLayer.setStyle(function (feature) {
    var co = feature.getProperty("company") || "";
    if (hiddenStations[co] || map.getZoom() < ZOOM_STATION)
      return { visible: false };
    return {
      icon: {
        path: google.maps.SymbolPath.CIRCLE,
  ...
  stationLayer.addListener("click", function (e) {
    ...
    document.getElementById("infoName").textContent =
      (e.feature.getProperty("name") || "") + " 駅";
    document.getElementById("infoDetail").textContent =
      displayCompanyName(e.feature.getProperty("company") || "") +
      " " +
      (e.feature.getProperty("line") || "");
    document.getElementById("infoMemo").style.display = "none";
    document.getElementById("info-bar").style.display = "block";
  });
  ...
  document
    .getElementById("railwayInput")
    .addEventListener("change", handleRailwayFiles);
  document
    .getElementById("stationInput")
    .addEventListener("change", handleStationFiles);
  loadFromCache();
  loadUserMarkers();
};
  ...
  reader.onload = function (ev) {
    ...
    parseJson(rawText)
      .then(function (geojson) {
        ...
        var count = geojson.features ? geojson.features.length : 0;
        document.getElementById("status").textContent =
          file.name + " - " + count.toLocaleString() + " 個のフィーチャー";
        buildLegend(geojson);
        addMunicipalityLabels(geojson);
      })
      .catch(function (err) {
...
function buildRailwayToggles() {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    html +=
      ...
      '<label title="路線図"><input type="checkbox"' +
      rC +
      ' data-company="' +
      items[i].name +
      '" class="route-toggle">線</label>' +
      '<label title="駅"><input type="checkbox"' +
      sC +
      ' data-company="' +
      items[i].name +
      '" class="station-toggle">駅</label>' +
      "</span>" +
      '<span class="co-color" style="background:' +
      items[i].color +
      '"></span>' +
      '<span class="co-name" data-company="' +
      items[i].name +
      '" title="' +
      dn +
      '">' +
  ...
  for (var j = 0; j < routeCbs.length; j++) {
    routeCbs[j].addEventListener("change", function () {
      ...
      if (this.checked) delete hiddenRoutes[name];
      else hiddenRoutes[name] = true;
      refreshLayers();
    });
  }
  for (var j = 0; j < stationCbs.length; j++) {
    stationCbs[j].addEventListener("change", function () {
      var name = this.getAttribute("data-company");
      if (this.checked) delete hiddenStations[name];
      else hiddenStations[name] = true;
      refreshLayers();
    });
  }

  document
    .getElementById("toggleAllRoutes")
    .addEventListener("click", function () {
      ...
      for (var k = 0; k < routeCbs.length; k++) {
        ...
        if (allR) delete hiddenRoutes[name];
        else hiddenRoutes[name] = true;
      }
      refreshLayers();
    });
  ...
  document
    .getElementById("toggleAllStations")
    .addEventListener("click", function () {
      ...
      for (var k = 0; k < stationCbs.length; k++) {
        ...
        if (allS) delete hiddenStations[name];
        else hiddenStations[name] = true;
      }
      refreshLayers();
    });
  ...
  for (var j = 0; j < coNames.length; j++) {
    coNames[j].addEventListener("mouseenter", function () {
      ...
      railwayLayer.forEach(function (f) {
        if (f.getProperty("name") === co)
          railwayLayer.overrideStyle(f, { strokeWeight: 4, strokeOpacity: 1 });
      });
    });
    coNames[j].addEventListener("mouseleave", function () {
      ...
      railwayLayer.forEach(function (f) {
        if (f.getProperty("name") === co) {
          if (selectedFeature === f && selectedLayer === "railway")
            applySelectionStyle(f, "railway");
...
function buildLegend(geojson) {
  ...
  for (var i = 0; i < geojson.features.length; i++) {
    ...
    items.push({
      code: code,
      label: [p.N03_003, p.N03_004].filter(Boolean).join(" ") || code,
      color: getColor(code),
    });
  }
  items.sort(function (a, b) {
  ...
  var html =
    ...
    grayChecked +
    ' style="margin-right:4px;cursor:pointer;">グレーで表示</label>';
  for (var j = 0; j < items.length; j++) {
    html +=
      '<div class="legend-item" data-code="' +
      items[j].code +
      '">' +
  ...
  for (var k = 0; k < legendItems.length; k++) {
    ...
    legendItems[k].addEventListener("mouseleave", function () {
      ...
      map.data.forEach(function (f) {
        if (f.getProperty("N03_007") === code) {
          if (selectedFeature === f && selectedLayer === "admin")
            applySelectionStyle(f, "admin");
    ...
    legendItems[k].addEventListener("click", function () {
      ...
      if (first) {
        ...
        var p = first;
        var parts = [
          p.getProperty("N03_001"),
          p.getProperty("N03_002"),
          p.getProperty("N03_003"),
          p.getProperty("N03_004"),
        ].filter(Boolean);
        document.getElementById("infoName").textContent = parts.join(" ");
        document.getElementById("infoDetail").textContent =
          "行政区域コード: " + (p.getProperty("N03_007") || "-");
        document.getElementById("infoMemo").style.display = "none";
        document.getElementById("info-bar").style.display = "block";
      }
    });
...
function addMunicipalityLabels(geojson) {
  labelOverlays.forEach(function (o) {
    o.setMap(null);
  });
  labelOverlays = [];
  var groups = {};
  ...
  for (var code in groups) {
    ...
    for (var k = 1; k < g.rings.length; k++) {
      if (g.rings[k].length > largest.length) largest = g.rings[k];
    }
...
function findVisualCenter(ring) {
  ...
  var bestX = (minLng + maxLng) / 2,
    bestY = (minLat + maxLat) / 2,
    bestWidth = -1,
    steps = 24;
